    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if len(data_validation_errors) > 0:
                return Response(
                    data_validation_errors, status=status.HTTP_400_BAD_REQUEST
                )
        try:
            # Create user
...
class TestAPIView(GenericAPIView):
    ...
    def get(self, request):
        data = {"message": "This is a test response from the TestAPIView."}
...
class LoginAPIView(APIView):
    ...
    def post(self, request):
        ...
        if not id_token:
            return Response(
                {"detail": "ID token is required."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            return Response(
                {"detail": f"Invalid ID token: {err}"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        if not sub:
            return Response(
                {"detail": "Invalid ID token."}, status=status.HTTP_401_UNAUTHORIZED
            )
...
class QuoteCreateAPIView(CustomAPIView):
    ...
    def post(self, request, *args, **kwargs):
        print(
            f"Authenticated user: {request.user}"
...
class JobListAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            print(
                f"Decoded token payload: {decoded_token.payload}"
            )  # Debugging: Print the token payload

            sub = decoded_token.get("sub")
            if not sub:
                return Response(
                    {"error": "Token does not contain a subject"}, status=400
                )
...
class QuoteDeleteAPIView(APIView):
    def delete(self, request, pk, *args, **kwargs):
        try:
            ...
            quote.save()
            return Response(
                {"message": "Quote deleted successfully."}, status=status.HTTP_200_OK
            )
...
class JobUpdateAPIView(UpdateAPIView):
    ...
    def put(self, request, pk, *args, **kwargs):
        try:
            ...
            if job.is_deleted:
                return Response(
                    {"error": "Cannot update a deleted job."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        except Job.DoesNotExist:
            return Response(
...
class AgreeQuoteAPIView(APIView):
    def post(self, request, pk, *args, **kwargs):
        try:
            ...
            return Response(
                {"error": "Quote not found or not in a valid state to be accepted."},
                status=status.HTTP_404_NOT_FOUND,
            )

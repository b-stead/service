...
class UserRegistrationSerializer(serializers.ModelSerializer):
    ...
    company_name = serializers.CharField(max_length=255, required=False, write_only=True)

    class Meta:
        model = User
        fields = ['email', 'password', 'first_name', 'last_name', 'is_company', 'company_name']
    ...
    def validate(self, data):
        ...
        if data.get('is_company') and not data.get('company_name'):
            raise serializers.ValidationError({"company_name": "This field is required for company registration."})
        ...
        with transaction.atomic():
            ...
            if is_company:
                ...
                if Company.objects.filter(owner=user).exists():
                    raise serializers.ValidationError({"company": "This user already owns a company."})
